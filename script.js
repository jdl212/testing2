...
static searchLastCity = function(e){
  Weather.clearWeatherInfo();
  if(Weather.SEARCH_HISTORY_LIST && Weather.SEARCH_HISTORY_LIST.length){
    Weather.clearWeatherInfo();
    Weather.searchCity(Weather.SEARCH_HISTORY_LIST[0]);
  }
}
static searchBtnOnClick = function (e) {
    ...
    if (cityName !== "" && Weather.SEARCH_HISTORY_LIST.length  && Weather.SEARCH_HISTORY_LIST.indexOf(cityName)>=0) {
      Weather.SEARCH_HISTORY_LIST = Weather.SEARCH_HISTORY_LIST.filter(function(city){ return city.toUpperCase()!=cityName.toUpperCase()});
